...
import { evaluateExams, generateRandomStudentAnswers, type EvaluationMode } from "./exams.service.js";
...
examsRouter.post(
  ...
  (req, res, next) => {
    try {
      ...
      const mode = evaluationModeSchema.parse(
        String(req.body.mode ?? "STRINGENT").trim().toUpperCase()
      ) as EvaluationMode;
