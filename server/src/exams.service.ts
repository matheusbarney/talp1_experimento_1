...
function parseCsv(csvBuffer: Buffer) {
...
function parseAnswerSheet(csvBuffer: Buffer): ParsedAnswerSheet[] {
...
function parseStudentAnswers(csvBuffer: Buffer): ParsedStudentAnswer[] {
  const rows = parseCsv(csvBuffer);
...
function decodePowerSumToSet(value: string, labels: string[]): Set<string> {
...
function parseSelection(answer: string, optionLabels: string[], answerLooksNumeric: boolean): Set<string> {
...
function randomSubset<T>(items: T[]) {
...
export function evaluateExams(answerSheetBuffer: Buffer, studentAnswersBuffer: Buffer, mode: EvaluationMode) {
  ...
  const reportRows: Array<(string | number)>[] = [];

  const perStudentScores: number[] = [];
  ...
  for (const student of students) {
    ...
    for (let questionIndex = 0; questionIndex < key.questions.length; questionIndex += 1) {
      ...
      if (mode === "LIBERAL" && !hasOptionLabels) {
        throw new Error(
          "Answer sheet is missing Q*_OPTIONS columns required for LIBERAL evaluation"
        );
      }

      const answerLooksNumeric =
        hasOptionLabels && answerEntry.optionLabels.every((label) => /^\d+$/.test(label));
...
export function generateRandomStudentAnswers(answerSheetBuffer: Buffer, studentCount: number) {
  const answerSheet = parseAnswerSheet(answerSheetBuffer);
  ...
  for (const exam of answerSheet) {
    for (const question of exam.questions) {
      if (question.optionLabels.length === 0) {
        throw new Error(
          "Answer sheet is missing Q*_OPTIONS columns required to generate random student answers"
        );
      }
    }
  }
